...
def get_gl_entries(filters, accounting_dimensions):
	...
	# Always add payment_method field
	payment_method_field = ", IFNULL(pe.payment_method, '') as payment_method"
	payment_method_join = "LEFT JOIN `tabPayment Entry` pe ON gle.voucher_no = pe.name AND gle.voucher_type = 'Payment Entry'"
	...
	gl_entries = frappe.db.sql(
		...
		select
			...
			gle.against, gle.is_opening, gle.creation {select_fields} {payment_method_field}
...
def get_conditions(filters):
	...
	if filters.get("against_voucher_no"):
		conditions.append("gle.against_voucher=%(against_voucher_no)s")
	...
	if filters.get("voucher_no_not_in"):
		conditions.append("gle.voucher_no not in %(voucher_no_not_in)s")
	...
	if not (
		...
		if not ignore_is_opening:
			conditions.append("(gle.posting_date >=%(from_date)s or gle.is_opening = 'Yes')")
		else:
	...
	if filters.get("include_default_book_entries"):
		if filters.get("finance_book"):
			...
			conditions.append("(gle.finance_book in (%(company_fb)s, '') OR gle.finance_book IS NULL)")
	else:
		if filters.get("finance_book"):
			conditions.append("(gle.finance_book in (%(finance_book)s, '') OR gle.finance_book IS NULL)")
		else:
			conditions.append("(gle.finance_book in ('') OR gle.finance_book IS NULL)")

	if not filters.get("show_cancelled_entries"):
		conditions.append("gle.is_cancelled = 0")

	if filters.get("debit_greater_than"):
		conditions.append("gle.debit > %(debit_greater_than)s")
...
def get_columns(filters):
	...
	# Always add Payment Method column after Posting Date
	# Find the index of the Posting Date column
	posting_date_index = next((i for i, col in enumerate(columns) if col.get("fieldname") == "posting_date"), 1)
	# Insert Payment Method column right after Posting Date
	columns.insert(posting_date_index + 1, {"label": _("Payment Method"), "fieldname": "payment_method", "fieldtype": "Data", "width": 120})
